...
public class PlayerMovementScript : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    private void Awake()
    {
        ...
        cameraPosition = cameraMain.localPosition;
        photonView = GetComponent<PhotonView>(); // Initialize photonView here
    }
	...
	*/
    private void Update()
    {
        ...
        {
            // Call methods only on the local player
            Jumping();
            // Other local player-specific logic
        }
        else
        {
            // If this is not the local player, update position and rotation from network data
